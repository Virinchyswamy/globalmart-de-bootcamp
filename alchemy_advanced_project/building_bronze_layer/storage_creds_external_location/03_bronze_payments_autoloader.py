...
# MAGIC %md
# MAGIC # Bronze Layer — Payments Ingestion via Autoloader
# MAGIC **GlobalMart | Tredence DE Advanced Training**
# MAGIC
# MAGIC | | |
# MAGIC |---|---|
# MAGIC | **Source** | ADLS Gen2 → External Location → `payments/` |
# MAGIC | **External Location** | `abfss://ecom-gbmart-data@ecomadlsdata.dfs.core.windows.net/raw-data` |
# MAGIC | **Target** | `gbmart.bronze.payments` (Managed Delta Table) |
...
# MAGIC
# MAGIC 1. Upload Day 1 files to `payments/` in ADLS
# MAGIC 2. Run Step 4 → **~42,000 rows** land in Bronze
# MAGIC 3. Upload `payments_020626.csv` to ADLS
# MAGIC 4. **Re-run Step 4** — Autoloader skips already-processed files via checkpoint, picks up only the new file
# MAGIC 5. Row count → **~126,000** with no duplicates
