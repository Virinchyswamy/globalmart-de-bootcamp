...
underlying idea**. They are not interchangeable, and mixing them mid-document
is a real mistake, not a stylistic choice — a learner who reads "quantity" in
one Scenario and `Quantity_purchased` in the next Input will assume they're
different columns.

## The decision rule

**The moment a hands-on Input writes real code against `gbmart` (a
`spark.table(...)` call, a `SELECT` with real column names, a schema diagram
meant to match what Catalog Explorer will actually show) — use the
REAL/TECHNICAL schema below, every time, regardless of which day it is.**
...
real code has appeared. Once real code appears in a scenario, don't go back to
simplified language even in the surrounding markdown — a learner who's about
to open `gbmart.gold.fact_sales` needs to see `Sales_amount`, not
"line_total".

## Simplified / story version

Source: `globalmart_problem_statement_architecture (1).html`, Section 5. Used
for Day 1's opening motivational framing — no hands-on scenario past that
point should use this vocabulary once it starts touching real columns.
...
**Never SUM or AVG `Actual_price`/`Discounted_price` directly** — they're a
per-line-item snapshot, not something that means anything summed across rows.
`Quantity_purchased` and `Sales_amount` are safe to SUM/AVG.
...
not a mistake to "fix" if you see it, and not something to silently correct
in a generated hands-on scenario.
...
Never write a hands-on question or solution that joins them.
...
does not mean `fact_sales` joins `dim_orders.order_sk` at any point. It
doesn't. The only *table* `fact_sales`'s build actually joins beyond
...
> `dim_orders` — and don't propagate the slide's looser phrasing into a new
> hands-on scenario.
...
Any hands-on Input asking a learner to join `fact_sales`/`order_items` to
`address` needs to call out this same collapsing step, or the join will
...
learners have felt why a full rebuild gets wasteful at scale — don't have a
fresh Day 7-style hands-on jump straight to MERGE.
...
every analyst writing their own join. If a hands-on scenario's purpose is to
serve "the actual deliverable" to business users, asking the learner to build
a view is more realistic than querying `fact_sales` directly — but that's a
modeling choice for the session at hand, not a rule this file enforces.
