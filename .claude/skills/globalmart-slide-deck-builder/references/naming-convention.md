...
workbook via `scripts/read_calendar.py` (the notebook-builder skill's
script — this skill does not bundle its own copy, since it never needs to
recompute `{order}`/`{x}` itself; see "How this skill actually uses this
file" at the bottom).
...
Day{N}/
  ...
  Day{N}_{order}_ILT{x}_Topic_Name.html     # the slide-deck explainer (built by this skill)
  Day{N}_{order}_HOL{x}_Topic_Name.ipynb    # hands-on lab (learners rebuild this themselves)
  Day{N}_{order}_HOL{x}_Topic_Name.html     # HOL instructions, same visual style (built by this skill)
...
This skill builds `.html` slide decks for `ILT{x}` and `HOL{x}` sessions only,
and only for a session whose `.ipynb` already exists. DEMO and REF files exist
in the real repo (see below) but this skill does not build decks for them — if
a user asks for a DEMO or REF deck, say so and ask how they want to proceed
rather than silently building an ILT/HOL deck in its place.
...
`scripts/read_calendar.py --day N` (bundled with the notebook-builder skill)
computes `{order}` for you, but only relative to that day's **calendar-derived**
ILT/Hands-on rows. Real DEMO/REF files are never in the calendar at all, and
they still consume order slots on disk — see the Day 3 worked example below
for exactly how this plays out. **Always Glob the real `Day{N}\` folder and
reconcile the script's number against what's actually sitting there before
finalizing a filename** — treat the script's output as the right relative
order among calendar-derived sessions, not the final absolute number,
whenever a DEMO or REF file might already exist that day.
...
## How this skill actually uses this file

Everything above is real and useful for *understanding* the naming scheme and
sanity-checking a filename you're handed — but this skill (the slide-deck
builder) is downstream of `globalmart-notebook-builder`, which already did
the hard work of computing `{order}`/`{x}` correctly (reconciling DEMO/REF
insertions, merged calendar rows, cross-day spillover, and known rule
exceptions) when it named the session's `.ipynb`. **Never re-run that
algorithm yourself.** Instead, Glob for the sibling `.ipynb` that already
exists on disk and derive this deck's filename directly from its basename
(swap `.ipynb` → `.html`). Re-deriving `{order}`/`{x}` independently risks
silently disagreeing with the notebook that already claimed that slot — worse,
it could produce a filename that doesn't match its own notebook at all, which
breaks the whole point of the naming convention (sibling files sorting
together). If the sibling `.ipynb` doesn't exist yet, that's the signal to
stop and tell the user to run `globalmart-notebook-builder` first — not a
reason to compute a filename from this doc's algorithm as a workaround.
