...
A slide about a Bronze/Silver session and a slide about a Gold session are
naming two genuinely different catalogs — don't normalize them to one name.
...
**What learners see on a slide vs. what a code snippet may show — the
placeholder convention.** Slide prose stays generic (`<your-catalog>`,
`YOUR_STORAGE_ACCOUNT_NAME`) since each student's own values differ. A
`slide-demo` code block may show GlobalMart's own real literal (`gbmart` or
`harsh_kumar01_npmentorskool_onmicrosoft_com`) if it's grounded in the
matching notebook's real code, with a one-line note that this is the real run
and the student's own catalog will be named differently — never a raw
storage-account key.
...
architectural fact anywhere, including in your own draft slide, stop and fix
it.
...
still say "7 real Bronze tables" / list only 6 total. If a slide you're
building needs this number, use 8 (confirmed against the real Day 4 build
notebooks); if you're specifically fixing one of those three files, flag the
discrepancy to the human rather than silently rewriting it as a side effect
of this skill.
