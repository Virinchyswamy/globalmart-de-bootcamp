...
underlying idea**. They are not interchangeable, and mixing them mid-notebook
is a real mistake, not a stylistic choice — a learner who reads "quantity" in
one cell and `Quantity_purchased` in the next will assume they're different
columns.

## The decision rule

**The moment a notebook writes real code against `gbmart` (a `spark.table(...)`
...
real code has appeared. Once real code appears in a session, don't go back to
...
for Day 1's opening motivational framing — no notebook past that point should
use this vocabulary once it starts touching real columns.
...
Never write code or narrative that joins them.
...
> `dim_orders` — and don't propagate the slide's looser phrasing into new
> notebooks.
...
Any notebook joining `fact_sales`/`order_items` to `address` needs this same
collapsing step, or the join will silently fan out rows. This is exactly the
kind of thing the row-count-after-every-join pattern in
`notebook-structure-patterns.md` is designed to catch.
...
learners have felt why a full rebuild gets wasteful at scale — don't jump to
MERGE for a fresh Day 7-style build.
...
every analyst writing their own join. If a notebook's purpose is to serve
"the actual deliverable" to business users, building a view is more realistic
than querying `fact_sales` directly — but that's a modeling choice for the
session at hand, not a rule this file enforces.
