...
colleague's (sayli's) own account all touch. A notebook that's technically
correct but writes to the wrong place, triggers a real job, or bakes in a
credential is worse than useless — it can break class for everyone else
mid-session, or leak something it shouldn't. Every rule below has a real,
verbatim code snippet already living in this repo — copy the *pattern*, not
necessarily the exact variable names, into any new write-heavy notebook.
...
it should be able to cost money or disrupt a real running system just by
being executed during a demo.

- Jobs/Workflow config is always an **inspectable, clearly-commented-as-illustrative
  Python dict**, constructed and printed — never submitted to a real Jobs API.
- Any streaming `.trigger(...)` either uses `availableNow=True` (terminates on
  its own) or explicitly calls `.stop()` before the cell finishes. A real gap
  of exactly this kind (a `.start()` with no matching `.stop()`) was found and
  fixed in Day 3's cert-prep notebook during a safety audit — that notebook is
  now the confirmed-fixed example, not a cautionary one.
...
- `VACUUM ... RETAIN 0 HOURS` never appears anywhere in this course (it
  permanently deletes file history) — any VACUUM shown is illustrative/
  commented-out only.
...
This skill (and every notebook it produces) **has zero live access to
...
comment, or notebook claim implying a live check happened.
...
HISTORY`, and plain `SELECT`s (all non-destructive, read-only).
...
example that makes this rule concrete, not hypothetical.
...
real-looking fake one that could be mistaken for live.
...
this skill that means: run `scripts/validate_ipynb.py <path>` after writing,
and actually look at its output, before telling the user the notebook is
ready.
