...
# MAGIC %md
# MAGIC
# MAGIC The address field contains an **actual newline character** inside the quoted string. Without `multiLine=True`, Spark treats that newline as a row separator — so SUP-05 is read as two rows:
# MAGIC - Row 5: `SUP-05, Miller Industries, ..., Chennai, ...`  ← everything before the newline
# MAGIC - Row 6: `Chennai 600002"` with nulls for all other columns ← everything after it
# MAGIC
# MAGIC This means you have **11 rows instead of 10**, and one supplier record is broken.
# MAGIC
# MAGIC **Fix:** Add `.option("multiLine", "true")` and `.option("escape", '"')` to the reader — shown in Step 2.
# MAGIC
