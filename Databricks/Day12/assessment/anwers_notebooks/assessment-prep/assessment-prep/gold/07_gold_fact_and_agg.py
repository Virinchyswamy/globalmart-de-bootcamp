...
spark.table("gbmart.gold_as.agg_carrier_performance") \
    .select("carrier_name", "on_time_pct", "avg_cost_inr", "total_shipments") \
    .orderBy(desc("on_time_pct")).display()

spark.table("gbmart.gold_as.agg_tier_analysis") \
    .select("tier_name", "total_shipments", "avg_actual_cost_inr", "on_time_pct_delivered") \
    .orderBy("tier_cost_inr").display()


# COMMAND ----------

# MAGIC %md
# MAGIC ### Carrier Performance

# COMMAND ----------

# MAGIC %sql
# MAGIC SELECT carrier_id, carrier_name, total_shipments, avg_delay_days, avg_cost_inr, on_time_pct
# MAGIC FROM gbmart.gold_as.agg_carrier_performance
...
# MAGIC %sql
# MAGIC SELECT shipping_tier_id, tier_name, total_shipments, tier_cost_inr, avg_actual_cost_inr, on_time_pct_delivered
# MAGIC FROM gbmart.gold_as.agg_tier_analysis
# MAGIC ORDER BY tier_cost_inr