...
# MAGIC %md
# MAGIC # Bronze Layer — Payment Methods Ingestion via Autoloader
# MAGIC **GlobalMart | Tredence DE Advanced Training**
# MAGIC
# MAGIC | | |
# MAGIC |---|---|
# MAGIC | **Source** | ADLS Gen2 -> External Location -> `payment_methods/` |
# MAGIC | **External Location** | `abfss://ecom-gbmart-data@ecomadlsdata.dfs.core.windows.net/raw-data` |
# MAGIC | **Target** | `gbmart.bronze.payment_methods` (Managed Delta Table) |
# MAGIC | **Auth** | Unity Catalog External Location — no keys in code |
# MAGIC | **Schema** | PaymentMethodID, MethodName, Provider, IsActive, TransactionFeePercent, MaxTransactionLimit |
# MAGIC | **Role in Star Schema** | Feeds `dim_payment_method` in Gold layer |
...
payment_methods_df = spark.readStream\
        .format("cloudFiles")\
...
payment_methods_df.writeStream\
        .format("delta")\
        .outputMode("append")\
