...
prerequisites:
  - Completed Day 7 ILT 2 — Performance in Modelling (Z-ORDER column selection from real query patterns)
  - Completed Day 4 HOL 2 — first exposure to OPTIMIZE/ZORDER/Liquid Clustering mechanics on a generic practice table
...
tags:
  ...
  - data-storage / liquid-clustering (skill)
  - data-storage / query-performance (skill)
---

---

## Scenario 1 — Build a Fully Isolated Practice Copy

**Overview:** Every experiment in this hands-on involves running `OPTIMIZE`, `ZORDER`, and `CLUSTER BY` — operations that rewrite data files. Running any of these against the real, shared `gbmart.gold.fact_sales` would affect every other learner's and the instructor's next query, with no way to undo it except a full rebuild. Before touching anything, you will isolate yourself completely: a one-time `SHALLOW CLONE` of the three tables this hands-on needs, into a schema only you own. After this scenario, nothing you do for the rest of this hands-on reads or writes `gbmart.*` again — everything happens inside your own practice schema.

**Outcome:** Three practice tables (`fact_sales_practice`, `dim_product_practice`, `dim_date_practice`) sitting in your own schema, containing the exact same data as the real Gold tables right now, with your own independent transaction history.
...
# HOW TO GET A SCHEMA NAME: use a catalog/schema you already have CREATE
# permission on (ask your instructor if unsure), and pick something unique
# to you, e.g. main.virinchy_zorder_lab.
...
spark.sql(f"CREATE OR REPLACE TABLE {FACT_TABLE}    SHALLOW CLONE gbmart.gold.fact_sales")
spark.sql(f"CREATE OR REPLACE TABLE {PRODUCT_TABLE} SHALLOW CLONE gbmart.gold.dim_product")
spark.sql(f"CREATE OR REPLACE TABLE {DATE_TABLE}    SHALLOW CLONE gbmart.gold.dim_date")
...
**Overview:** A freshly-cloned table is already reasonably well-organized, because it inherited the real table's current file layout — that's not representative of what `fact_sales` will look like after months of Day 9–10's incremental `MERGE`-based refresh, where each day's run typically adds a handful of new, small files. Rather than waiting months, you will deliberately re-fragment your practice copy into many small files, reproducing that end state on demand.

**Outcome:** A practice `fact_sales_practice` table split across exactly 200 files, ready to be measured, queried, and optimized.
...
## Scenario 3 — Baseline Three Real Query Shapes

**Overview:** A single query timing only tells you about one access pattern. `fact_sales` is queried at least three different ways in real GlobalMart reporting: a category/month revenue rollup (`vw_monthly_category_sales`'s shape), a state/city regional rollup (`vw_regional_sales`'s shape), and a narrow point lookup for one specific product over a date range (the shape a customer-support or merchandising tool would run). Before touching `OPTIMIZE` or `CLUSTER BY` at all, you will time all three against the fragmented baseline table, so every later comparison has a real "before" to compare against.

**Outcome:** Three baseline timings recorded, plus a written prediction of which query you expect to benefit most from clustering, made before you have any results to bias your reasoning.
...
spark.sql(f"CREATE OR REPLACE TABLE {PRACTICE_SCHEMA}.dim_address_practice SHALLOW CLONE gbmart.gold.dim_address")
...
## Scenario 4 — OPTIMIZE + Z-ORDER

**Overview:** Day 7 ILT 2 identified `Product_ID` and `Time_ID` as the right Z-ORDER columns for `fact_sales` — not a guess, a conclusion drawn from the real business queries that actually run against it (the same two query shapes you just baselined). In this scenario, you will apply `OPTIMIZE ... ZORDER BY` using those columns and re-measure all three queries to see exactly where the improvement shows up, and where it doesn't.

**Outcome:** A compacted, Z-ordered practice fact table, plus three "after" timings to compare directly against Scenario 3's baseline.
...
## Scenario 5 — Liquid Clustering, and What Happens When New Data Lands

**Overview:** Z-ORDER is a one-time, full-table rewrite — useful, but every subsequent `OPTIMIZE` re-evaluates and can re-shuffle the *entire* table again. `fact_sales` isn't a one-time table: Day 9–10 refreshes it incrementally, adding new order lines continuously. Liquid Clustering (`CLUSTER BY`) is designed specifically for that pattern — it clusters incrementally, only touching the files that actually need it. This scenario builds a Liquid-clustered twin of your practice table, then proves the incremental-refresh difference empirically by simulating a small batch of new orders landing and timing how long each table takes to re-cluster afterward.

**Outcome:** A second practice table built with `CLUSTER BY`, its own set of the same three query timings, and a direct, measured comparison of Z-ORDER's vs. Liquid Clustering's incremental `OPTIMIZE` cost after new data arrives.
...
## Scenario 6 — Final Comparison and Recommendation

**Overview:** With every timing captured across all three query shapes and both clustering techniques, plus the incremental re-optimize cost measured directly, you now have everything needed to make (and defend) a real recommendation for `fact_sales` specifically — not clustering in the abstract.

**Outcome:** One consolidated comparison table and a written, evidence-based recommendation, ready to submit alongside your completed notebook.
